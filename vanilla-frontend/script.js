/* ══════════════════════════════════════════
   CareerSprout — Main Application JavaScript
...
async function handleSignup() {
    ...
    try {
        ...
        updateNavForUser();
        closeAuthModal();
        showToast(`Welcome to CareerSprout, ${data.user.firstName}! 🚀 Start exploring now.`, 'success');
        navigate('jobs');
    } catch (err) {
        showToast('Connection error. Please try again.', 'error');
    }
...
function openApplyModal(index) {
    ...
    if (!job) return;
    state._lastJobIndex = index;  // store for LinkedIn fallback
    ...
    const token = localStorage.getItem('jn_token') || '';
    fetch(`http://localhost:5000/api/jobs/verify`, {
        method: 'POST',
...
function applyLinkedIn() {
    const job = state.jobs[state._lastJobIndex];
    const url = state.jobLinkedInUrl
        || (job ? `https://www.linkedin.com/jobs/search/?keywords=${encodeURIComponent(job.title + ' ' + job.company)}` : null);
...
async function runATSScreening() {
    ...
    try {
        ...
        const res = await fetch(`https://job-nexus-production.up.railway.app/api/ats/screen`, { method: 'POST', body: formData });
        const data = await res.json();
...
async function generateResume() {
    ...
    try {
        const res = await fetch(`https://job-nexus-production.up.railway.app/api/resume/latex`, {
...
async function generateLinkedInContent() {
    ...
    try {
        const res = await fetch(`https://job-nexus-production.up.railway.app/api/linkedin/tips`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
